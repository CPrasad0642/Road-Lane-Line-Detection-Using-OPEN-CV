...
def make_coordinates(image,line_parameters):
    ...
    y1 = image.shape[0]  # Height of an image.
    y2 = int(y1 * (3/5))# 3/5 th of the height of an image
...
def average_slope_intercept(image,lines):
    ...
    for line in lines:
        ...
        if slope<0:
            left_fit.append((slope,intercept)) #left fit contains left lanes
        else :
            right_fit.append((slope,intercept))#right fit contains a right lanes
    ...
    left_fit_average = np.average(left_fit,axis=0)
    right_fit_average = np.average(right_fit,axis=0)
    left_line = make_coordinates(image,left_fit_average)#Average of left fit
    right_line = make_coordinates(image,right_fit_average)#Average of right fit
    return np.array([left_line,right_line])


def canny(image):
    gray = cv2.cvtColor(image,cv2.COLOR_RGB2GRAY)#
    blur = cv2.GaussianBlur(gray,(5,5),0) #5X5 kernel
    canny = cv2.Canny(blur,50,150)# 50 and 150 are low_threshold and high_threshold
    return canny

def display_lines(image,lines):
    line_image = np.zeros_like(image)#we create black background
...
def region_of_interest(image):
    ...
    masked_image = cv2.bitwise_and(image,mask) # bitwise and operation is performed b/w mask and canny image of each homologous pixel
    return masked_image
...
while(cap.isOpened()):
    _, frame = cap.read() #each frame is read
    if frame is None:
        break
